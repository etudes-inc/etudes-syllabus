/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/syllabus/trunk/syllabus-app/src/java/org/sakaiproject/tool/syllabus/SyllabusTool.java $
 * $Id: SyllabusTool.java 9891 2015-01-23 18:03:11Z mallikamt $
 ***********************************************************************************
 *
 * Copyright (c) 2003, 2004, 2005, 2006, 2015 The Sakai Foundation.
 * 
...
public class SyllabusTool
{
  public class DecoratedSyllabusEntry
  {
    ...
    {
      in_entry = en;
      id = en.getSyllabusId().toString() + ",";
    }

	public String getCleanAsset()
	{
		String cleanText = "";
		if (in_entry != null) cleanText = in_entry.getAsset();
		if (cleanText != null) cleanText = HtmlHelper.clean(cleanText, true);
		return cleanText;
	}
  ...
  private Boolean redirectUrlHttpsPrefix = null;
  ...
  private String mapBlocked  = null;

  private String displayBlockMsg = null;
  
  protected String allowUser = null;
  
  protected String returnAddr = null;

  protected Date viewDate = null;
  
  protected String collBase = null;
  
  protected String resourceFile = null;
  
  protected int entriesSize = 0;
  
  protected String newOrder = null;
  
  protected String oldPosition = null;
  
  protected String newPosition = null;
  
  protected boolean openNewWindow = false;
  
  protected boolean showSave = false;
  
  protected String testOpenWindow = null;
  ...
  public SyllabusTool()
  {
   ...
   public String getNewOrder() {
	...
	/**
	 * Get the open in new window value as string for testing on main_view page
	 * This value is used for rendering iframe. if returned value is null then iframe doesn't render.
	 * @return
	 */
	public String getTestOpenWindow()
	{
		if (this.openNewWindow )
			testOpenWindow = null;
		else
			testOpenWindow = new Boolean(this.openNewWindow).toString();
		
		return testOpenWindow;
	}
...
public boolean getdisplayNoEntryMsg()
  ...
  public int getEntriesSize() {
	  if (entries != null) entriesSize = entries.size();
	  else entriesSize = 0;
...
public DecoratedSyllabusEntry getEntry()
  ...
  public SyllabusItem getSyllabusItem() throws PermissionException
  {
    ...
    try
    {
      ...
      openNewWindow = (syllabusItem != null) ? syllabusItem.isOpenNewWindow() : false;
...
public String getUserId()
  ...
  /**
   * Checks if user has access to the syllabus.
   * Only instructors, guest, student and TA's can access it.
   * @return true if accessAllowed
   */
  public String checkUser()
  {
	  ...
	  {
		  // check for student or TA
		  Placement placement = ToolManager.getCurrentPlacement();
		  String currentSiteId = placement.getContext();
		  boolean guestOrStudent = SecurityService.unlock("section.role.student", "/site/" + currentSiteId);
		  // if user is student allow access
		  allowUser = (guestOrStudent) ? "true" : "false";
		  // if not check for TA
		  if (guestOrStudent == false)
		  {
			  boolean ta = SecurityService.unlock("section.role.ta", "/site/" + currentSiteId);
			  allowUser = (ta) ? "true" : "false";
		  }
...
//          }
    ...
    try
    {
      ...
      else
      {
        ...
        if(entry.getEntry().getAsset()!=null)
        {
        	...
        	String assetData = HtmlHelper.clean(entry.getEntry().getAsset(), true);
        	entry.getEntry().setAsset(assetData);
        }
        entry.getEntry().setEmailNotification("none");
  ...
  public String processRedirect() throws PermissionException
  {
    try
    {
      ...
      else
      {
    	...
    	openNewWindow = syllabusItem.isOpenNewWindow();
    	  ...
    	  if (currentRediredUrl != null && !"".equals(currentRediredUrl)) {
    		  if (currentRediredUrl.indexOf(httpPrefix) == -1 && currentRediredUrl.indexOf(httpsPrefix) == -1 ) {
    			  if (ServerConfigurationService.getToolUrl().indexOf(httpsPrefix) != -1) {
    				  currentRediredUrl = httpsPrefix + currentRediredUrl; 
    			  }
    			  else  currentRediredUrl =  httpPrefix + currentRediredUrl;			
    		  }		  		 
    	  }   	  
    	  
      	currentRediredUrl = currentRediredUrl.replaceAll("\"", ""); 
    	  syllabusItem.setRedirectURL(currentRediredUrl);
    	  syllabusItem.setOpenNewWindow(openNewWindow);
        syllabusManager.saveSyllabusItem(syllabusItem);
  ...
  /**
   * Delete the url instead of making user put in blank url
   * @return
   * @throws PermissionException
   */
  public String processUrlDelete() throws PermissionException
  {
    try
    {
      if (!this.checkAccess())
      ...
      else
      {
    	  currentRediredUrl = "";	  
     	  syllabusItem.setRedirectURL(currentRediredUrl);
     	  syllabusItem.setOpenNewWindow(false);
     	  syllabusManager.saveSyllabusItem(syllabusItem);
    ...
    {
   //   logger.info(this + ".processUrlDelete in SyllabusTool: " + e);
      FacesContext.getCurrentInstance().addMessage(
          null,
          MessageFactory.getMessage(FacesContext.getCurrentInstance(),
              "error_permission", (new Object[] { e.toString() })));
...
//for test        entry.getEntry().getAsset(), null, null, null);
  ...
  /**
   * returns true if link to url starts with https:// 
   * @return
   */
	public Boolean getRedirectUrlHttpsPrefix()
	{
		redirectViewURL = getRedirectViewURL();
		if (redirectViewURL == null || redirectViewURL.length() == 0)
		{
			redirectUrlHttpsPrefix = null;
			return redirectUrlHttpsPrefix;
		}

		if (redirectViewURL != null && redirectViewURL.startsWith(httpsPrefix))
			redirectUrlHttpsPrefix = new Boolean(true);
		else if (redirectViewURL != null && redirectViewURL.startsWith(httpPrefix))
			redirectUrlHttpsPrefix = new Boolean(false);
		else
			redirectUrlHttpsPrefix = null;
	...
	/**
	 * Get the redirect url for view pages
	 * @return
	 */
	public String getRedirectViewURL()
	{
		try
		{
			...
			if (syItem != null)
			{
				redirectViewURL = syItem.getRedirectURL();
				if (redirectViewURL != null && !"".equals(redirectViewURL))
				{
					if (redirectViewURL.indexOf(httpPrefix) == -1 && redirectViewURL.indexOf(httpsPrefix) == -1)
					{
						if (ServerConfigurationService.getToolUrl().indexOf(httpsPrefix) != -1)
						{
							redirectViewURL = httpsPrefix + redirectViewURL;
						}
						else
							redirectViewURL = httpPrefix + redirectViewURL;
					}
				}
			} else redirectViewURL = "";
		}
		catch (PermissionException e)
		{
			// logger.info(this + ".getRediredUrl() in SyllabusTool " + e);
			FacesContext.getCurrentInstance().addMessage(null,
					MessageFactory.getMessage(FacesContext.getCurrentInstance(), "error_permission", (new Object[]
					{ e.toString() })));
		}
...
/**
  ...
  /**
   * get status attribute of syllabus
   * @return
   */
  public String getSyllabusDataStatus()
  {
	  String rv = "";
	  DecoratedSyllabusEntry entry = getEntry();
	  boolean alert = true;
  ...
  public String getSyllabusDataAsset()
  {
	  ...
	  if (entry != null)
	  {
		  ...
		  if (syllabusData != null)
		  {
			  ...
			  if (rv != null) rv = HtmlHelper.clean(rv, true);
			  alert = false;
		  }
		  collBase = contentHostingService.getSiteCollection(siteId);
		  String attrb = "ck.security.advisor." + collBase;
		  ...
		  resourceFile = "Syllabus_"+ syllabusData.getSyllabusId() + ".html";
	  }
  ...
  /*
   * Checks if syllabus is blocked by coursemap
   *
   * returns false if not blocked
   */
  public String getMapBlocked ()
  {
	  if(mapBlocked  == null)
	  {
		  String currentSiteId = ToolManager.getCurrentPlacement().getContext();
		  String blockedBy = syllabusService.isSyllabusBlocked(currentSiteId);
		  if(blockedBy != null && blockedBy.trim().length() != 0) mapBlocked  = "TRUE";

		  // for instructors show the block message and the content
		  if(blockedBy != null && blockedBy.trim().length() != 0 && checkAccess()) mapBlocked  = null;
	  }
	  return mapBlocked ;
  }

  public void setMapBlocked(String mapBlocked) {
	this.mapBlocked  = mapBlocked ;
}

	/*
	 * Checks if syllabus is blocked by coursemap If its blocked then show the message to the user
	 * 
	 * returns null if not blocked
	 */
	public String getDisplayBlockMsg()
	{
		if (displayBlockMsg == null)
		{
			// not for instructors
			if (!checkAccess())
			{
				...
				if (blockedBy != null && blockedBy.trim().length() != 0)
				{
					displayBlockMsg = rb.getString("blocked") + " " + syllabusService.blockedDetails(currentSiteId) + ":<ul><li>" + blockedBy + "</li></ul>";
				}
			}
		}

		return displayBlockMsg;
	}

  public void setDisplayBlockMsg(String displayBlockMsg) {
	this.displayBlockMsg = displayBlockMsg;
 }

 /*
  * Saves acceptance if user clicks on Yes.
  */
  public String agree()
  {
	// for instructors we are not saving
	//if (checkAccess()) return "main_view";
	
    String agreeUserId = SessionManager.getCurrentSessionUserId();
  
    Placement placement = ToolManager.getCurrentPlacement();
	String currentSiteId = placement.getContext();
	
	syllabusManager.saveSyllabusAcceptance(currentSiteId, agreeUserId);
	showAgreement = true;
 ...
 /*
  * If user hasn't accepted terms of syllabus then show the box
  * Returns false for instructors and for user's who haven't accepted yet
  */
  public boolean isShowAgreement() {
	  // for instructors its always showing and we are not fetching from db
	  if(checkAccess())
	  {
	  ...
	  else
	  {
		  if(showAgreement == null)
		  {
			  ...
			  Placement placement = ToolManager.getCurrentPlacement();
			  String currentSiteId = placement.getContext();
			  showAgreement = syllabusManager.isSyllabusAcceptedByUserAndContextId(currentSiteId, agreeUserId);
		  }
	  }
	  return showAgreement.booleanValue();
  }
	...
	/*
	 * construct jforum send pm url
	 */
	public String getSendPMUrl()
	{
		//do any settings reqd by jforum to call this page
		...
		try
		{
			...
			pmUrl = ServerConfigurationService.getPortalUrl() + "/directtool/" + currentSite.getToolForCommonId("sakai.jforum.tool").getId() + "/pm/send.page";
		}
		catch (Exception e){}
		return pmUrl;
	}
	
	public boolean getRenderAgreementBox()
	{
		boolean renderBox = false;
		renderBox = !isShowAgreement(); 

		//if blocked then don't show
		if (mapBlocked != null) 
			renderBox = renderBox && false;

		// if no syllabus then don't show
		try
		{
			if(displayNoEntryMsg && (getSyllabusItem().getRedirectURL() == null || getSyllabusItem().getRedirectURL().trim().length() == 0))
				renderBox = renderBox && false;
		}		
		catch(Exception e){ renderBox = false;}

		return renderBox; 
	}
	
	/**
	 * Set coursemap return address
	 * 
	 * @param returnAddr
	 *        The return address
	 */
	...
	{
		returnAddr = returnAddr.replace("/!portal!/", "/portal/directtool/");
		String returnUrl = ServerConfigurationService.getServerUrl() + returnAddr;
		returnAddr = null;
		return returnUrl;
	}
	
	/**
	 * track syllabus visits
	 */
	public Date getViewDate()
	{
		String viewUserId = SessionManager.getCurrentSessionUserId();
		Placement placement = ToolManager.getCurrentPlacement();
		String currentSiteId = placement.getContext();
		String accessBlocked = syllabusService.isSyllabusBlocked(currentSiteId);
		try
		{
			if (accessBlocked == null && (!displayNoEntryMsg || (getSyllabusItem().getRedirectURL() != null && getSyllabusItem().getRedirectURL().trim().length() > 0)))
				syllabusManager.trackSyllabusVisitsByUserId(currentSiteId, viewUserId);
		}
		catch (Exception e)
		{
	...
	public void setCollBase(String collBase) {
	...
	/**
	 * If the reorder save button should show on the page. For link to url option and empty entries it doesn't show.
	 * @return
	 */
	public boolean isShowSave()
	{
		try
		{
			SyllabusItem s = getSyllabusItem();
			showSave = (s.getRedirectURL() == null || s.getRedirectURL().length() == 0) ? true : false;
			showSave = showSave && ((entries == null || entries.size() == 0) ? false : true);
		}
		catch (Exception e)
		{
			showSave = false;
	...
	/**
	 * invoked when user clicks Save and uses drag drop to reorder items
	 * @return
	 */
	public String savePositions()
	{
		if (!this.checkAccess())
	      ...
	      if(newOrder == null || newOrder.length() == 0) return "main_edit";
	      ArrayList<SyllabusData> sds = new ArrayList<SyllabusData> (0);
	      try
			{
	       SyllabusItem s = getSyllabusItem();            
	        // replace with syllabusManager.getSyllabusItemByContextId(currentSiteId);
	       
	        Set<SyllabusData> tempEntries = syllabusManager.getSyllabiForSyllabusItem(s);
	        
			String[] parts = newOrder.split(",");
			for (String p : parts)
			{
				Long findId = new Long(p);
				for(SyllabusData sd:tempEntries)
				{
					if (sd.getSyllabusId().equals(findId))
						{
						sds.add(sd);
						break;
						}
			...
			syllabusManager.sortSyllabusDataPositions(sds);
			...
			{
				logger.info(this + ".savePositions() in SyllabusTool " + e);
			}
	...
	/**
	 * On reordering using combo box, send the old and new position items to the server.
	 * It will move all of the in-between items up or down.
	 * @return
	 */
	public String saveToNewPositions()
	{
		if (!this.checkAccess())
		{
			return "permission_error";
		...
		try
		{
			int o = Integer.parseInt(oldPosition);
			int n = Integer.parseInt(newPosition);

			SyllabusItem s = getSyllabusItem();
			Set<SyllabusData> sdItems = syllabusManager.getSyllabiForSyllabusItem(s);
			SyllabusData[] items = sdItems.toArray(new SyllabusData[0]);
			if (n > items.length || o < 1) return "main_edit";

			SyllabusData moveItem = items[o - 1];

			syllabusManager.moveSyllabusDataToNewPosition(s, moveItem, items[n - 1]);
		...
		{
			logger.info(this + ".saveToNewPositions() in SyllabusTool " + e);
		}
